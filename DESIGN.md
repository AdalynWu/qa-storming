...
| `public/rpg-library-celestial.png` | 賢者書庫桌機背景 | 深藍綠星穹典藏殿、直線柱廊、邊緣書架、黃銅星盤與左／右內容安全區 |
| `public/rpg-library-celestial-mobile.png` | 賢者書庫手機背景 | 同一典藏殿的直式 art direction；上方星空與中央通道保留標題、CTA、卷冊堆疊空間 |
...
| `public/rpg-realm-dawn-valley.png` | 探索之森林領域景色 | 晨光瀑布谷、中央河道與金色遠山；中央窄裁切仍保留景深 |
| `public/rpg-realm-crystal-canyon.png` | 守門者峽谷領域景色 | 月光水晶峽谷、銀色瀑布與藍紫極光；中央水道維持門洞焦點 |
| `public/rpg-realm-ember-plain.png` | 直播星火原領域景色 | 暮色熔岩原、玄武岩台地與遠方火山；亮部集中於中央路徑 |
| `public/rpg-realm-sunken-garden.png` | 內容秘藏地領域景色 | 藍綠沉沒庭園、鏡池、石柱與發光水生花；中央保留遠景層次 |
| `public/rpg-product-world-map.png` | 產品世界地圖 | 與生命樹一致的繪本筆觸、自然地貌與金色日光 |
| `public/rpg-quest-card.png` | 魔法任務卡 | 對稱黃銅卡框、藤蔓、寶石與羊皮紙安全區；用於新手村任務輪播 |
| `public/rpg-quest-workshop.png` | 新手村桌機背景 | 暖色室內公會製圖工坊、直線木構、矩形高窗與長形地圖桌；中央保留任務卡軌道空間 |
| `public/rpg-quest-workshop-mobile.png` | 新手村手機背景 | 同一工坊的直式 art direction；上方保留標題暗部，製圖桌與中央卡片維持完整層次 |
...
<p>
  ...
  <img src="./public/rpg-quest-card.png" alt="魔法任務卡" width="24%">
</p>
...
- 桌機導覽第四個入口「迷霧測試林」(`/rpg`) 為**桌機專屬**，採**純 CSS 能力 gate**(畫面 ≥1024×600 且非純觸控才顯示，避免 hydration 位移)，不放進手機收合選單；真正的裝置判斷與鍵盤確認在 `/rpg` 內。
...
- 底部「向下探索」提示不使用底色或外框；箭頭、文字與整組控制器必須共用 viewport 水平中心，並以亮色前景、深色陰影與足夠垂直間距維持插畫上的可讀性，同時保留清楚的鍵盤 focus 樣式。

### 新手村魔法任務卡

- 場景背景使用 `rpg-quest-workshop.png`／手機直式版本，以室內公會製圖工坊區隔首屏生命樹與下一屏森林遺跡；不再覆用生命樹或森林丘陵光霧。
- 工坊以直線木樑、矩形高窗、地圖櫃、黃銅儀器與長形製圖桌建立水平骨架；背景圖不可內嵌卡片、文字、箭頭或 UI。
- 任務輪播的視覺物件使用 `rpg-quest-card.png`；保留原書封的深森林琺瑯、藤蔓、黃銅、寶石與羊皮紙語言，但不呈現書脊、書頁或厚度。
- 桌機 `.zone-title` 直接依 section 的 `6vw` 內距定位，與試煉之森 `.portal-title` 共用左側文字基準；標題不可包進置中的書環內容容器。
- 卡面 HTML 文字安全區以原圖羊皮紙為準，約為卡片水平 `24%–76%`；分類、標題與說明不可延伸到左右藤蔓，CTA 可獨立落在下方裝飾區。
- 卡面不顯示進度／獎勵列；資訊層級固定為分類、標題、簡短說明與單一 CTA，避免與主要入口競爭。
- 卡面 CTA 文案固定單行；字級與箭頭間距應隨卡片寬度縮放，不得以換行容納窄尺寸。
- 任務方向鍵與卡面 CTA 使用深森林琺瑯、黃銅描邊及書扣／金屬鉸鏈感；圖示使用可縮放 SVG，不用一般亮色圓鈕、Unicode 箭頭或通用膠囊按鈕。
- 桌機區段以 `100dvh - navbar` 作為一個完整場景；標題、五張可見卡位、方向控制與目前位置必須同時落在可視範圍內。應依 viewport 高度流動縮放卡片與間距，不得用裁切隱藏溢出內容。
- `≥2000px` 的超寬桌機需同步放大卡片、軌道半徑與控制間距，維持互動主體的視覺份量；不可只擴張背景而讓中央卡退化為小型裝飾。
- 任務卡以既有偽 3D 圓形軌道呈現；一次只移動一格並可無限循環，不改變目前方向與進場動畫。
- 中央卡片最大、最亮且正面朝向使用者；hover 或鍵盤 focus 時以單一連續的金綠漸層光團回應，不可疊加矩形 drop-shadow 形成上下斷層，也不改變尺寸或位置。
- 卡片 PNG 目前為裁切後的 811×1318 母檔，卡片比例固定約 1:1.625；羊皮紙水平安全區約為 19%–82%，HTML 文字採左右約 23% 內距。互動光暈使用內縮的透明 radial-gradient 並柔化邊緣，避免依矩形畫布產生可見接縫。日後再次手動裁切 PNG，必須同步重跑 `npm run optimize:images` 產生 AVIF／WebP，並重新校準卡片比例與文字 padding。
- 左右卡片正面朝向外側觀者，不應只露出側邊或背面。
- 最多顯示前方五個位置；超過五張時其餘位於軌道後方並隨切換進場。
- 實際任務不足五張時，以不可點擊的封印卡補足視覺環。
- 只有中央有效卡片的 CTA 可以執行連結；封印卡不可點擊。
...
- 四個 realm 各自使用獨立 `rpg-realm-*` 純風景，不覆用生命樹、產品地圖或內容截圖；圖內不可包含角色、文字、UI 或傳送門框。
- Realm 風景以 `<picture>` 依 AVIF → WebP → PNG 順序提供同內容格式，中央 45% 必須同時適用桌機圓形門洞與手機直式裁切。
...
- 六隻不同尺寸與拍翼相位的蝙蝠編隊作為試煉場景的遠景生命感，桌機與手機起點皆位於場景寬度約 70%；桌機垂直起點約 24%、手機約 14%，讓編隊保持在傳送門上半部並與下方巨龍拉開距離。飛行角度保持平緩，不穿越主要標題或右下操作 HUD，reduced-motion 下停止位移與拍翼。

### 賢者書庫

- 場景使用 `rpg-library-celestial.png`／手機直式版本，以深藍綠石材、直線柱廊、邊緣書架、黃銅星盤與開放星空構成「星穹典藏殿」；避免覆用工坊木造室內或森林傳送門構圖。
- 亮色文件卡片像典藏條目；桌機背景左側與右側內容區保持低對比，手機中央縱向通道需容納標題、CTA 與卷冊堆疊。
- Three.js 星群只作裝飾性漸進增強：書庫接近 viewport 且瀏覽器空閒後才載入，游標在場景內時產生低幅度星圖牽引與視差；離屏／隱藏分頁停止 render loop，手機降低粒子數，reduced-motion 不載入 canvas。
...
- 閱讀頁 Topbar 使用完整且可理解的階層麵包屑：「首頁 > 產品地圖 > 產品 > 目前章節」；父層皆為可操作連結，目前章節使用 `aria-current="page"`。窄螢幕允許麵包屑自身水平捲動，不得撐破頁面或用難懂世界觀詞彙取代資訊架構。
- 產品 Hub 桌機可使用彎曲路徑；手機改為清楚的雙欄或單欄章節格，不把桌機地圖等比例縮小。

### 賢者書庫閱讀器

- 目錄、正文與相關札記使用具名 grid areas 表達閱讀順序；寬桌機三欄、窄桌機先讓目錄獨佔一列、手機依目錄 → 正文 → 相關札記完整單欄排列。
- 裝飾徽章內的大字與副標採縱向 flow 與明確 gap，不用重疊 grid 製造層次；內容在字體 fallback、縮放與翻譯後仍不得互相覆蓋。
...
- 全站只保留左下角 Chatbot 的六幀史萊姆嚮導，不在首頁場景重複出現第二隻同角色；關閉時由角色斜右上方的深森林／黃銅對話框提示「有什麼疑問可以問我唷」，泡泡可壓入 sprite 的透明畫布但不可碰到實際可見的史萊姆，且不使用額外尾巴。點擊角色後由左下錨點展開非 modal 的羊皮紙問答工作台。桌機不遮住主要內容焦點，手機面板完整落在 viewport 安全區內。
- 層級固定為櫃台身分／公開資料範圍、訊息紀錄、建議問題、loading／error、來源與輸入區；回答來源使用原生 `<details>`，連回網站公開閱讀頁，不顯示本機 Markdown 路徑或任意模型 URL。
- Assistant 與使用者訊息需以位置、標籤、材質與文字共同區分，不只依賴顏色。回答使用純文字與換行呈現，不直接渲染模型輸出的 HTML。
- 人工核准的趣味問題以本地精準比對處理，顯示「史萊姆閒聊」標籤與淡綠羊皮紙材質，不顯示知識來源，也不呼叫搜尋索引或 Gemini；近似但未核准的問法一律回到正式知識庫流程，避免把玩笑回答誤套到工作問題。
- 入口、關閉與送出皆至少 44px；支援 Enter 送出、Shift+Enter 換行、Escape 關閉、明確 focus、live region，以及送出期間的 disabled 防重複操作。
- 史萊姆入口沿用等距格 `sprite-jelly`，不另製重複資產；邀請對話框只在出現時由角色端短促展開一次，面板開啟只保留一次書頁展開感，loading 使用低幅度節奏。`prefers-reduced-motion` 下停在第一幀並取消對話框、展開與等待動畫，但保留即時狀態文字。

### 迷霧測試林(`/rpg` 3D 教學副本)

- 全視窗沉浸式 3D 遊戲，以同源 `<iframe>` 隔離載入自帶 three.js 的完整遊戲；不改寫成 React、不共用站上 three。上方保留一條站內品牌 topbar(深色橋接遊戲色調、pink 主色、`← 返回世界樹` 在 iframe 之前以利鍵盤返回)。
- **桌機專屬**：進入前先以複合條件判斷(畫面 ≥1024×600 且非純觸控)。合格→顯示原生按鈕「按 Enter 或點擊進入迷霧測試林」，確認後才載入 iframe；不合格→顯示擋頁卡片「此副本需要實體鍵盤與足夠的畫面空間」＋「我有鍵盤，仍要進入」覆寫入口，且**不載入 iframe** 以保護手機效能。
...
- 首屏以下的 Three.js 場景必須由自身 section 的 Intersection Observer 控制延遲 dynamic import，不可因首頁 hydration 立即下載；啟用後仍沿用 DPR 上限、離屏／隱藏暫停與完整資源釋放規則。
...
- 緩慢漂浮：書本、果實、光點。
- 差速視差：背景、霧、葉片與角色。
- 試煉之森蝙蝠以六隻不同尺寸、不同拍翼相位組成鬆散編隊，隨捲動往右方微幅上升；整群皆為裝飾且在 reduced-motion 下停止位移與拍翼。
