...
| `public/rpg-trial-portal.png` | 試煉之森背景 | 深色遺跡森林、中央傳送門、既有石台留作控制區 |
...
新增場景圖片時，優先沿用 `1672 × 941`、約 `16:9` 的橫幅比例，並在桌機與手機裁切下保留主要焦點。圖片需先壓縮，在不明顯破壞插畫筆觸的前提下降低檔案大小。
...
- 直接使用背景插畫既有石台作控制器；符文與能量是透明互動疊層，不新增第二個石製輪盤圖片。
- 切換需有逐格定位、越過定位點後回彈與短暫重量震動。
- 領域名稱、說明、Module、案例數、上一個／下一個與 CTA 集中在右下資訊組。
- 桌機資訊組不可遮擋傳送門與石台；手機移到場景下方或安全區。
...
- 緩慢漂浮：書本、果實、光點。
- 差速視差：背景、霧、葉片與角色。
- 機械重量：石台逐格轉動、越位回彈、卡榫震動。
- 魔法切換：光紋、符文依序亮起、霧氣與景色過渡。
...
- Carousel、石台與地圖需同時支援滑鼠、觸控與鍵盤。
- Drag 超過明確門檻才切換，放開後一次只移動一格。
