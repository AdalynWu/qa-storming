...
export function QuestBookCarousel({
  ...
  const slides =
    quests.length >= MIN_LOOP_SLIDES
      ? quests
      : Array.from(
          { length: Math.ceil(MIN_LOOP_SLIDES / quests.length) },
          () => quests,
        ).flat();
  ...
  return (
    ...
    >
      ...
      <Swiper
        ...
        a11y={{
          enabled: true,
          prevSlideMessage: "上一項任務",
          nextSlideMessage: "下一項任務",
          ...
          <SwiperSlide key={`${quest.id}-${index}`}>
            {({ isActive }) => (
              ...
              >
                ...
                <div className="book-corner corner-one" aria-hidden="true">
                  ◆
                </div>
                <div className="book-corner corner-two" aria-hidden="true">
                  ◆
                </div>
      ...
      <p className="quest-position" aria-live="polite">
        {(activeIndex % quests.length) + 1} / {quests.length}
      </p>
    </div>
  );
}
