...
> 📚 **開工前務必先讀治理文件**:root 的 [`AGENTS.md`](AGENTS.md)(跨 agent 工作守則、文件索引)與 `docs/` 下的 `PLAN.md`、`PROGRESS.md`、`TASK.md`(+ 相關 `DECISION.md`、`ARCHITECTURE.md`、`MEMORY.md`)。UI 任務另讀 root [`DESIGN.md`](DESIGN.md)。**完成工作後更新 `docs/PROGRESS.md`**;架構變更更新 `docs/DECISION.md`;milestone 更新 `docs/PLAN.md`。本專案由 Claude 與 Codex 並行開發,兩邊都可能改動檔案。
...
- **樣式:** Tailwind CSS v4(`@import "tailwindcss"`)+ `src/app/globals.css` 手寫的 RPG 主題。
...
- `npm run start` — 目前 script 仍是 `next start`,但不適用 `output: "export"`;不要用它驗證靜態輸出。請用 Firebase Hosting Emulator 或一般靜態伺服器開 `out/`。
- `npm run lint` — ESLint(忽略 `out`、`.next`)。
- `npm run deploy` — **一步完成部署**:`next build` 後 `firebase deploy --only hosting`。**不需要**另外先跑 `npm run build`。

## 架構重點

- `src/app/` — 網站本體(App Router)。
  ...
  - `globals.css` — 完整 RPG 主題(CSS 變數 + scroll-driven transform)。
- `src/components/` — 共用元件(`QuestBookCarousel`、`TrialForestPortal`)。
- `src/content/` — 內容資料模組(`quests.ts`、`products.ts`、`regression.ts` + `generated/regression.json`)。
- `scripts/` — 建置工具(`sync-regression.ts` 從 Google Sheet 同步 → `src/content/generated/regression.json`)。留在 root。
...
- `public/` — 靜態資產:`rpg-life-tree.png`(hero 背景 + OG 圖)、favicon、sprite sheets。**Next 要求 `public/` 必須在 root。**
- 路徑別名:`@/*` → `src/`(見 tsconfig)。

## 重要地雷(先看這段)

- **首頁內容是寫死的靜態陣列。** `src/app/page.tsx` 內的 `fruits` / `quests` / `lore` 三個陣列是硬寫在檔案裡的資料,並**不是**從 Markdown 或 DB 載入(即使頁面文案宣稱「以 Markdown 保存」,尚未實作)。要改文件卡片內容 → 改 `src/app/page.tsx`。
...
- Git:目前 repository metadata 指向 `main`;agent 不執行任何 Git 指令,分支與版控操作由使用者處理。
