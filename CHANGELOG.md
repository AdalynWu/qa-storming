...
### Changed
- `/rpg` 迷霧測試林題庫擴充為 13 盞（每局隨機抽 6、保證至少一盞白盒覆蓋，新增巢狀/決策表/迴圈邊界/狀態轉換/靜態 vs 動態/殺蟲劑悖論/預期結果），結尾清單改列本局實際抽中的盞；品牌色由桃紅 `#FF2D78` 改為青綠 `#7ED6D6`（含站內 topbar）；移除角色圍巾、燈光升高不照底座、地面指引光點加大加亮、左臂改上臂貼身手外開；`/rpg` 遊戲頁不再顯示全站 Chatbot（其他頁照常）。(2026-08-04)
- 修正 `/rpg` 迷霧測試林轉鏡頭(←→)後 WASD 未跟隨視角、W 會往下的移動向量符號錯誤,改為正確的相對鏡頭移動(任意角度 W 皆往畫面前方)。(2026-08-04)
- 修正 `/rpg` 迷霧測試林 iframe 高度塌陷造成遊戲標題裁切與無法開始(`#start` 溢出視窗),iframe 改 `position:absolute; inset:0` 撐滿定值高度外框並在焦點交接時 `preventScroll`；同時移除站內外框與遊戲標題畫面的 vibe-coding 字樣。(2026-08-04)
